...
def train_rf_single_feature(X_feature, y, feature_name, dataset_name):
    """
    Train Random Forest on a single feature and return metrics
    """
    # Split the data
    ...
    y_pred_proba = rf.predict_proba(X_test)[:, 1] if len(np.unique(y)) == 2 else None
    ...
    if len(np.unique(y)) == 2 and y_pred_proba is not None:
        try:
            ...
            print(f"  Warning: Could not calculate ROC AUC for feature {feature_name}
            roc_auc = None
    
    # Cross-validation score
    cv_scores = cross_val_score(rf, X_feature.values.reshape(-1, 1), y, 
    ...
    return {
        ...
        'Feature_Importance': rf.feature_importances_[0] if len(rf.feature_importances_) > 0 else None
    }

def process_dataset(file_path, dataset_name):
    """
    Process a dataset and train RF on each feature
    """
    ...
    # Train RF on each feature individually
    for feature in tqdm(feature_columns, desc=f"Processing {dataset_name} features", unit="feature"):
        ...
        # Train RF and get metrics
        try:
            metrics = train_rf_single_feature(X_feature, y, feature, dataset_name)
...
def main():
    ...
    """
    print("Starting Random Forest Single Feature Analysis...")
    ...
    if not results_df.empty:
        ...
        output_file_combined = os.path.join(output_folder, 'rf_single_feature_results_combined.csv')
        ...
        for dataset in results_df['Dataset'].unique():
            ...
            output_file = os.path.join(output_folder, f'rf_single_feature_results_{dataset}.csv')
