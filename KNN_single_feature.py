...
def train_knn_single_feature(X_feature, y, feature_name, dataset_name):
    """
    Train KNN on a single feature and return metrics
    """
    # Split the data
    ...
    # Find optimal k using GridSearchCV on a smaller parameter space for efficiency
    param_grid = {'n_neighbors': [3, 5, 7, 9, 11, 15]}
    
    knn_grid = KNeighborsClassifier(weights='distance', metric='euclidean')
    ...
    y_pred_proba = knn.predict_proba(X_test_scaled)[:, 1] if len(np.unique(y)) == 2 else None
    ...
    for train_idx, val_idx in skf.split(X_feature.values.reshape(-1, 1), y):
        ...
        cv_knn.fit(X_cv_train_scaled, y_cv_train)
        cv_pred = cv_knn.predict(X_cv_val_scaled)
...
def process_dataset(file_path, dataset_name):
    """
    Process a dataset and train KNN on each feature
    """
    ...
    # Train KNN on each feature individually
    for feature in tqdm(feature_columns, desc=f"Processing {dataset_name} features", unit="feature"):
        ...
        # Train KNN and get metrics
        try:
            metrics = train_knn_single_feature(X_feature, y, feature, dataset_name)
...
def main():
    ...
    """
    print("Starting KNN Single Feature Analysis...")
    ...
    if not results_df.empty:
        ...
        output_folder = 'KNN_Results'
        if not os.path.exists(output_folder):
            os.makedirs(output_folder)
            print(f"Created output folder: {output_folder}")
        
        # Save comparison results to CSV
        output_file_combined = os.path.join(output_folder, 'knn_single_feature_results_combined.csv')
        ...
        for dataset in results_df['Dataset'].unique():
            ...
            output_file = os.path.join(output_folder, f'knn_single_feature_results_{dataset}.csv')
        ...
        for dataset in results_df['Dataset'].unique():
            ...
            print(f"  Most common best K: {dataset_results['Best_K'].mode().iloc[0] if not dataset_results['Best_K'].mode().empty else 'N/A'}")
            print(f"  Mean best K: {dataset_results['Best_K'].mean():.1f}")
        ...
        # Display K value analysis
        print("\n" + "="*80)
        print("K-VALUE ANALYSIS")
        print("="*80)
        for dataset in results_df['Dataset'].unique():
            dataset_results = results_df[results_df['Dataset'] == dataset]
            k_distribution = dataset_results['Best_K'].value_counts().sort_index()
            print(f"\n{dataset} Dataset - Best K distribution:")
            for k, count in k_distribution.items():
                percentage = (count / len(dataset_results)) * 100
                print(f"  K={k}: {count} features ({percentage:.1f}%)")
