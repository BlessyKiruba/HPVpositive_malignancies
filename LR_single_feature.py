...
def train_lr_single_feature(X_feature, y, feature_name, dataset_name):
    """
    Train Logistic Regression on a single feature and return metrics
    """
    # Split the data
    ...
    # Find optimal regularization parameter using GridSearchCV
    param_grid = {
    ...
    grid_search = GridSearchCV(
        lr_grid, 
        param_grid, 
        cv=3,  # Reduced CV folds for single feature efficiency
        scoring='accuracy',
        n_jobs=-1
    )
    ...
    y_pred_proba = lr.predict_proba(X_test_scaled)[:, 1] if len(np.unique(y)) == 2 else None
    ...
    # Cross-validation score with scaling
    cv_scores = []
    skf = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)
    
    for train_idx, val_idx in skf.split(X_feature.values.reshape(-1, 1), y):
        X_cv_train, X_cv_val = X_feature.values[train_idx].reshape(-1, 1), X_feature.values[val_idx].reshape(-1, 1)
        ...
        cv_lr.fit(X_cv_train_scaled, y_cv_train)
        cv_pred = cv_lr.predict(X_cv_val_scaled)
    ...
    return {
        ...
        'Coefficient': lr.coef_[0][0] if hasattr(lr, 'coef_') else None,
        'Intercept': lr.intercept_[0] if hasattr(lr, 'intercept_') else None,
        'Grid_Search_Best_Score': grid_search.best_score_
    }

def process_dataset(file_path, dataset_name):
    """
    Process a dataset and train Logistic Regression on each feature
    """
    ...
    # Train Logistic Regression on each feature individually
    for feature in tqdm(feature_columns, desc=f"Processing {dataset_name} features", unit="feature"):
        ...
        # Train Logistic Regression and get metrics
        try:
            metrics = train_lr_single_feature(X_feature, y, feature, dataset_name)
...
def main():
    ...
    """
    print("Starting Logistic Regression Single Feature Analysis...")
    ...
    if not results_df.empty:
        ...
        output_folder = 'LR_Results'
        if not os.path.exists(output_folder):
            os.makedirs(output_folder)
            print(f"Created output folder: {output_folder}")
        
        # Save comparison results to CSV
        output_file_combined = os.path.join(output_folder, 'lr_single_feature_results_combined.csv')
        ...
        for dataset in results_df['Dataset'].unique():
            ...
            output_file = os.path.join(output_folder, f'lr_single_feature_results_{dataset}.csv')
        ...
        for dataset in results_df['Dataset'].unique():
            ...
            print(f"  Most common best C: {dataset_results['Best_C'].mode().iloc[0] if not dataset_results['Best_C'].mode().empty else 'N/A'}")
            print(f"  Most common penalty: {dataset_results['Best_Penalty'].mode().iloc[0] if not dataset_results['Best_Penalty'].mode().empty else 'N/A'}")
        ...
        # Display regularization analysis
        print("\n" + "="*80)
        ...
        for dataset in results_df['Dataset'].unique():
            ...
            c_distribution = dataset_results['Best_C'].value_counts().sort_index()
            print(f"\n{dataset} Dataset - Best C distribution:")
            for c, count in c_distribution.items():
                percentage = (count / len(dataset_results)) * 100
                print(f"  C={c}: {count} features ({percentage:.1f}%)")
            
            # Penalty distribution
            penalty_distribution = dataset_results['Best_Penalty'].value_counts()
            print(f"\n{dataset} Dataset - Best Penalty distribution:")
            for penalty, count in penalty_distribution.items():
                percentage = (count / len(dataset_results)) * 100
                print(f"  {penalty}: {count} features ({percentage:.1f}%)")
