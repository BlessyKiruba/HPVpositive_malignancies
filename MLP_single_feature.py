...
def train_mlp_single_feature(X_feature, y, feature_name, dataset_name):
    """
    Train MLP on a single feature and return metrics
    """
    # Split the data
    ...
    y_pred_proba = mlp.predict_proba(X_test_scaled)[:, 1] if len(np.unique(y)) == 2 else None
    ...
    for train_idx, val_idx in skf.split(X_feature.values.reshape(-1, 1), y):
        ...
        cv_mlp.fit(X_cv_train_scaled, y_cv_train)
        cv_pred = cv_mlp.predict(X_cv_val_scaled)
    ...
    return {
        ...
        'Loss': mlp.loss_ if hasattr(mlp, 'loss_') else None
    }

def process_dataset(file_path, dataset_name):
    """
    Process a dataset and train MLP on each feature
    """
    ...
    # Train MLP on each feature individually
    for feature in tqdm(feature_columns, desc=f"Processing {dataset_name} features", unit="feature"):
        ...
        # Train MLP and get metrics
        try:
            metrics = train_mlp_single_feature(X_feature, y, feature, dataset_name)
...
def main():
    ...
    """
    print("Starting MLP Single Feature Analysis...")
    ...
    if not results_df.empty:
        ...
        output_folder = 'MLP_Results'
        if not os.path.exists(output_folder):
            os.makedirs(output_folder)
            print(f"Created output folder: {output_folder}")
        
        # Save comparison results to CSV
        output_file_combined = os.path.join(output_folder, 'mlp_single_feature_results_combined.csv')
        ...
        for dataset in results_df['Dataset'].unique():
            ...
            output_file = os.path.join(output_folder, f'mlp_single_feature_results_{dataset}.csv')
        ...
        for dataset in results_df['Dataset'].unique():
            ...
            print(f"  Mean iterations: {dataset_results['N_Iterations'].mean():.1f}")
