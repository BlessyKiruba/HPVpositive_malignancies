...
def train_svm_single_feature(X_feature, y, feature_name, dataset_name):
    """
    Train SVM on a single feature and return metrics
    """
    # Split the data
    ...
    y_pred_proba = svm.predict_proba(X_test_scaled)[:, 1] if len(np.unique(y)) == 2 else None
    ...
    cv_scores = cross_val_score(pipeline, X_feature.values.reshape(-1, 1), y, 
    ...
    return {
        ...
        'Support_Vectors': len(svm.support_) if hasattr(svm, 'support_') else None
    }

def process_dataset(file_path, dataset_name):
    """
    Process a dataset and train SVM on each feature
    """
    ...
    # Train SVM on each feature individually
    for feature in tqdm(feature_columns, desc=f"Processing {dataset_name} features", unit="feature"):
        ...
        # Train SVM and get metrics
        try:
            metrics = train_svm_single_feature(X_feature, y, feature, dataset_name)
...
def main():
    ...
    """
    print("Starting SVM Single Feature Analysis...")
    ...
    if not results_df.empty:
        ...
        output_folder = 'SVM_Results'
        if not os.path.exists(output_folder):
            os.makedirs(output_folder)
            print(f"Created output folder: {output_folder}")
        
        # Save comparison results to CSV
        output_file_combined = os.path.join(output_folder, 'svm_single_feature_results_combined.csv')
        ...
        for dataset in results_df['Dataset'].unique():
            ...
            output_file = os.path.join(output_folder, f'svm_single_feature_results_{dataset}.csv')
